...
text(clean1$FEducation, clean1$Under5Mortality, labels = clean1$Country, cex = 0.7, pos = 4) #allows me to put country names so can check outlier
...
# intercept is the value of health if education was zero,so if women had 0 years of education, under 5 mortality would be 75.81 per 1000 live birth
# coefficient of education is -5.7247
# the p-value is lower than 0.05, therefore you can reject the H0 
# for every 1 year increase in female education, 5.7 less children under 5 will die per every 1000 children under 5
# o meglio detto A one-year increase in average female schooling is associated with:
# a decrease of 5.7 deaths per 1,000 live births in under-5 mortality
...
summary(firstreg2)
#A one-year increase in female schooling is associated with a 17.9% decrease in under-five mortality
...
#with the exact formula, the effect is -8.1% (-8.084453). A one-year increase in female schooling is associated with
#a 8.1% decrease in under-five mortality, holding GDP constant.
...
summary(model5) #its not significant anymore and R2 is at 6.9

(exp(-0.041470) - 1) * 100 
#-4.062188 for model 5
...
# Pvalue < 0.05, we to reject the null of homoscedasticity! bad news!
#The Breusch–Pagan test indicates the presence of heteroskedasticity if (p < 0.05) 
#because it is 0.06431 for model 5, we fail to reject the H0, we find no strong evidence of heteroscedasticity
...
(exp(-0.0348413 ) - 1) * 100
#just slightly smaller effect but same significance level

#SO BASICALLY CHECK THE DISTRIBUTION AND CORRELATIONS OF VARIABLES AND IF NEEDED LOG. THEN RUN MODELS. THEN CHECK OLS ASSUMPTIONS FOR THE ULTIMATE MODELS
#JUST AFTER VERYFYING THAT ASSUMPTIONS HOLD (OR FIXING ROBUST STANDARD ERRORS ETC ADD THOSE ONLY TO THE STARGAZER TABLE
#IF STRONG OUTLIERS, RUN NORMALLY BEFORE AND ONLY AFTER WE WILL TRY TO REMOVE OUTLIERS AND PUT IT IN ROBUSTNESS CHECKS (but put in the 
# stargazer the version with outliers)
...
#the plan is for robustness: in there we write everything I do basically: 
#different combinations of controls try education gap; add tax revenue
#the model without controls 
#When interpreting remember how many are low and how many are middle income to generalize with more confidence

#MOVING TO STUNTING 

secondreg <- lm(Stunted ~ FEducation, data= clean1)
summary(secondreg)
#one additional year of female education is associated with -2.56% in under 5 stunting

model1.2 <- lm(Stunted ~ FEducation + log_GDPpc, data = clean1)
summary(model1.2)

#“An additional year of female education is associated with a 1.25 percentage point reduction in child stunting.”
...
#The Breusch–Pagan test indicates the presence of heteroskedasticity if (p < 0.05)
#because it is 0.4705 for model 5.2, we fail to reject the H0, we find no strong evidence of heteroscedasticity
#or can say, the Breusch-Pagan test indicates no heteroskedasticity (0.4705 > 0.05)
...
#smaller effect and no significance anymore
...
#“An additional year of female education is associated with a 0.9 percentage point increase population that have access to water.”
...
# Pvalue < 0.05, therefore we to reject the null of homoscedasticity! bad news!
#The Breusch–Pagan test indicates the presence of heteroskedasticity if (p < 0.05)
#because it is 0.003856 for model 5.3, this indicates heteroskedasticity
#so we need to do robust standard errors
#in case of heteroscedasticity, easy fix:
library(sandwich)
...
#How you can say it :The Breusch–Pagan test indicates the presence of heteroskedasticity (p < 0.05). Therefore, robust standard errors are used in all regressions to ensure valid inference
...
#IT WILL BE CORRECTED ERRORS 5.3 THAT I HAVE TO INTERPRET AND THAT I HAVE TO INSERT IN THE FINAL STARGAZER
...
#for this reason better do stargazer after checking assumptions
#right order is 1)Run OLS model 2)Check assumptions (plots + tests beusch pagan, vif, outliers)
#3) Detect problems 4) Fix inference if needed (robust SE, etc.) 5)Interpret results
...
summary(fourthreg)
#one additional year of female education is associated with a 0.09 increase in the Government effectiveness score
...
#here there seems to be a 111 leverage point 
...
#NO heteroskedasticity (heteroscedasticity is when p < 0.05), bc the value is 0.8039
...
#bigger effect and still no significance 
